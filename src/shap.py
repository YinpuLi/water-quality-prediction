...
def gen_shap_results(
    ...
    if isinstance(best_model, (xgb.XGBRegressor, RandomForestRegressor, lgb.LGBMRegressor)):
        print("Tree Based Model...")
        explainer = shap.TreeExplainer(best_model)
        shap_values = explainer.shap_values(refit_X)
        shap.summary_plot(shap_values, refit_X, plot_type = 'bar', show=False)
        #plt.gcf().set_size_inches(15, 10)
        # Create the directory if it doesn't exist
        os.makedirs(os.path.dirname(save_file_path_1), exist_ok=True)
        
        plt.savefig(save_file_path_1, dpi=figure_dpi ,bbox_inches='tight')
        plt.show()
        
        shap.summary_plot(shap_values, refit_X, show=False)
        #plt.gcf().set_size_inches(15, 10)
        # Create the directory if it doesn't exist
        os.makedirs(os.path.dirname(save_file_path_2), exist_ok=True)
        
        plt.savefig(save_file_path_2, dpi=figure_dpi,bbox_inches='tight')
        plt.show()
        
    elif isinstance(best_model, (RandomForestRegressor)):
        print("Tree Based Model...")
      
        # randomly select n sample to calculate SHAP in rf. If not, it will be slow.
        np.random.seed(827)
        refit_X = refit_X.iloc[np.random.randint(0,len(refit_X),500)]
        ...
        # randomly select n sample to calculate SHAP in rf. If not, it will be slow.
        np.random.seed(827)
        refit_X_summary = shap.kmeans(refit_X, 37)
        refit_X = refit_X.iloc[np.random.randint(0,len(refit_X),100)]
...
def generate_file_paths_for_shap_2(
    ...
    best_model_file = path_ + "\\" + rel_path + "\\" + 'best_'+ model_name + '_model.joblib'
    
    best_shap_file_1 = path_ + "\\" + rel_path + "\\" + "shap_on_test" + "\\"  + 'best_' + model_name + '_shap_bar.png'
    
    
    best_shap_file_2 = path_ + "\\" + rel_path + "\\" + "shap_on_test" + "\\"  + 'best_' + model_name + '_shap_val.png'
